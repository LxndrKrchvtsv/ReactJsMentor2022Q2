...
export const ButtonOption = ({ openRemoveModal, openEditModal, movieEditId }: Props) => {
	...
	const handleSetIsOptionClose = (isClose: boolean) => (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
	...
	return (
		<div className={Styles.button__option__wrapper}>
			<div className={Styles.button__option} onClick={(event) => handleSetIsOptionClose(true)(event)}>
			...
			{isOptionClose && (
				<div className={Styles.option__item__wrapper} onClick={(event) => event.stopPropagation()}>
					...
					<span className={Styles.option__item} onClick={() => openRemoveModal(movieEditId)}>
						{'Remove'}
					</span>
				</div>
			)}
